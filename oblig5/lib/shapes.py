...
def height_to_color(height_value):
    """
    Convert height value to RGB color for terrain
    Returns: (r, g, b) tuple with values 0-255
    """
    # Define color stops for different terrain types
    if height_value < 0.2:    # Deep water
        return (0, 0, 139, 255)   # Dark blue
    elif height_value < 0.3:  # Shallow water
        return (30, 144, 255, 255)  # Dodger blue
    elif height_value < 0.4:  # Beach/sand
        return (238, 214, 175, 255)  # Sandy color
    elif height_value < 0.5:  # Grass/plains
        return (34, 139, 34, 255)   # Forest green
    elif height_value < 0.7:  # Forest/hills
        return (0, 100, 0, 255)     # Dark green
    elif height_value < 0.9:  # Mountains
        return (139, 137, 137, 255)  # Gray
    else:                   # Snow peaks
        return (255, 250, 250, 255)  # Snow white




class Planet(pyglet.shapes.ShapeBase):
    def __init__(
        self,
        x: float, y: float, z: float,
        vertices: list, indices: list, count: int,
        noise: list,
    ...
    def _create_colours(self):
        ...
        if self._noise is not None:
            for i in range(self._num_verts):
                c.extend(height_to_color(self._noise[i]))
        else:
            for i in range(self._num_verts):
                c.extend((255, 255, 255, 255))
        return c
